...
DIO_ERR_DLL_DEVICE_NAME = (
    10000  # Device name which isn't registered in setting file is specified.
)
...
DIO_ERR_INFO_INVALID_DEVICE = (
    10050  # Specified device name isn't found. Please check the spelling.
)
DIO_ERR_INFO_NOT_FIND_DEVICE = 10051  # The usable device isn't found.
DIO_ERR_INFO_INVALID_INFOTYPE = (
    10052  # The specified device information type is outside the range.
)
...
DIO_ERR_SYS_BOARD_EXECUTING = (
    20002  # It cannot perform because the device is executing.
)
DIO_ERR_SYS_USING_OTHER_PROCESS = (
    20003  # It cannot perform because the other process is using the device.
)
DIO_ERR_SYS_NOT_SUPPORT_KERNEL = (
    20004  # It is not supporting in the kernel of this version.
)
...
DIO_ERR_SYS_TIM = (
    20300  # Timer value is outside the setting range. Error in trigger function.
)
...
DIODM_STATUS_TRGIN = (
    0x8  # Indicates that the start signal is inserted by external start.
)
DIODM_STATUS_OVERRUN = (
    0x10  # Indicates that start signals are inserted twice or more by external start.
)
# ----------------------------------------
# Error
# ----------------------------------------
...
# ----------------------------------------
PDIO_INT_CALLBACK = ctypes.CFUNCTYPE(
    None, ctypes.c_short, ctypes.c_short, ctypes.c_long, ctypes.c_long, ctypes.c_void_p
)
PDIO_TRG_CALLBACK = ctypes.CFUNCTYPE(
    None, ctypes.c_short, ctypes.c_short, ctypes.c_long, ctypes.c_long, ctypes.c_void_p
)
PDIO_STOP_CALLBACK = ctypes.CFUNCTYPE(
    None, ctypes.c_short, ctypes.c_short, ctypes.c_long, ctypes.c_void_p
)
PDIO_COUNT_CALLBACK = ctypes.CFUNCTYPE(
    None, ctypes.c_short, ctypes.c_short, ctypes.c_long, ctypes.c_void_p
)
...
DioSetInterruptCallBackProc.argtypes = [
    ctypes.c_short,
    PDIO_INT_CALLBACK,
    ctypes.c_void_p,
]
...
DioDmSetInternalClock.argtypes = [
    ctypes.c_short,
    ctypes.c_ulong,
    ctypes.c_ulong,
    ctypes.c_ulong,
...
DioDmGetWritePointerUserBuf.argtypes = [
    ctypes.c_short,
    ctypes.c_ulong,
    ctypes.POINTER(ctypes.c_ulong),
    ctypes.POINTER(ctypes.c_ulong),
